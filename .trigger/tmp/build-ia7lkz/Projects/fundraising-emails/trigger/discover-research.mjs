    ...
    const response = await fetch("https://api.perplexity.ai/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a political news researcher. Write a numbered list of 5-10 recent, distinct news stories. For each item write: the headline, a 1-2 sentence summary, and cite your source using [number] notation matching the citations provided. Example format:\n1. **Headline here** — Summary of the story. [1]\n2. **Another headline** — Summary text. [3]"
          },
          {
            role: "user",
            content: query
...
function parseNumberedList(content, citations) {
  const results = [];
  ...
  for (const item of items) {
    ...
    if (boldMatch) {
      ...
      summary = item.replace(/\*\*(.+?)\*\*/, "").replace(/^\s*[—–-]\s*/, "").trim();
    } else {
...
__name(parseNumberedList, "parseNumberedList");
