...
var discoverResearch = schedules_exports.task({
  ...
  run: /* @__PURE__ */ __name(async () => {
    ...
    const { data: brandKits, error: bkError } = await supabase.from("brand_kits").select("user_id, kit_name, org_type, org_level, office_sought, state, district, brand_summary");
...
async function processUserDiscovery(kit) {
  ...
  const { data: existing } = await supabase.from("research_topics").select("source_url").eq("user_id", kit.user_id).order("created_at", { ascending: false }).limit(200);
  ...
  const topTopics = uniqueTopics.sort((a, b) => b.relevance_score - a.relevance_score).slice(0, 8);
  let saved = 0;
...
function buildSearchQueries(kit) {
  const queries = [];
  ...
  if (stanceKeywords.length > 0 && (office || state)) {
    ...
    const stanceParts = stanceKeywords.slice(0, 3).map((kw) => `"${kw}" AND ${context}`);
...
async function newsApiSearch(query) {
  ...
  try {
    ...
    return data.articles.filter((a) => a.title && a.title !== "[Removed]" && a.url).map((article) => {
    ...
    logger.error("NewsAPI search failed", { error: err.message });
    return [];
  }
}
__name(newsApiSearch, "newsApiSearch");
function scoreArticle(article, domain) {
  let score = 3;
  if (TIER_1.some((d) => domain.includes(d))) score += 4;
  else if (TIER_2.some((d) => domain.includes(d))) score += 2.5;
  else score += 1;
  if (article.publishedAt) {
    const hoursAgo = (Date.now() - new Date(article.publishedAt).getTime()) / (1e3 * 60 * 60);
...
__name(scoreArticle, "scoreArticle");
