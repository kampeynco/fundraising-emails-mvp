    ...
    const response = await fetch("https://api.perplexity.ai/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a political research assistant. Return ONLY a JSON array of objects with keys: title, summary, source_url. Each item should be a distinct news story or policy development. Return 5-10 items, sorted by relevance. No markdown, no explanation, just the JSON array."
          },
          {
            role: "user",
            content: query
